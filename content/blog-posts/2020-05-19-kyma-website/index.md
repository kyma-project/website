...
- [Deploy previews](https://www.netlify.com/blog/2016/07/20/introducing-deploy-previews-in-netlify/) built on pull requests, allowing you to detect any rendering issues before they are merged to the `master` branch.

- [AWS functions](https://docs.netlify.com/functions/overview/) for running on-demand code triggered by events coming from the GitHub API. In Kyma, we use it as a trigger for running the build and deployment of given website views. This function is triggered whenever you:
...
Before the website build, all this content is copied to the `website` repository by the [content loader](https://github.com/kyma-project/website/tree/master/tools/content-loader) - our in-house TypeScript tool we use for fetching:
...
4. Gatsby uses the [Node API](https://www.gatsbyjs.org/docs/node-apis/) to build particular static HTLM sites from React components, using data from GraphQL.

5. Gatsby [optimizes](https://www.gatsbyjs.org/docs/performance/) the static sites to increase their performance.

6. Netlify deploys the static sites.
