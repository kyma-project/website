...
```bash
  ...
  -d, --domain string          Domain used for installation. (default "kyma.local")
      --fallbackLevel int      If "source=latest-published", defines the number of commits from the main branch taken into account if artifacts for newer commits do not exist yet (default 5)
  ...
  -s, --source string          Installation source.
                               	...
                               	- To use the latest main, write "kyma install --source=latest".
                               	- To use the latest published main, which is the latest commit with released images, write "kyma install --source=latest-published".
