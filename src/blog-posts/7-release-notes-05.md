...
- The modularization of Application Connector has been added. All components have been moved to a separate Helm chart and are no longer part of the Core. It also includes improvements of the naming of elements. The gateway service has been renamed to the Proxy service.
- The Application Connector tests were extended with an additional flag for skipping SSL verification. It allows testing functionality against Kyma cluster with a self-signed certificate.
...
- Support for &quot;namespaced&quot; service catalog. This mean a possibility to register brokers for only specific namespaces. The Catalog has new additional view in the Environment context, called Brokers and it lists Service Brokers registered only for a given namespace.
  - [link](https://github.com/kyma-project/kyma/blob/master/docs/service-catalog/docs/050-gs-broker-registration.md) to new getting started guide about brokers registration
  - [link](https://github.com/kyma-project/kyma/blob/master/docs/service-catalog/docs/020-architecture-service-catalog.md) to architecture diagram about namespace catalog
- We now officially published our documentation of the contract we applied between catalog UI and the open service broker API specification that is a core of Service Catalog:
...
- Instance list and details views now work on web-sockets. This significantly improves user experience because all changes to the instance - most important its status - is updated in real-time in the view without a need for a page refresh. If you are interested how we managed to accomplished that on a GraphQL layer with Istio and token verification, feel free to contact us on [Slack](https://join.slack.com/t/kyma-community/shared_invite/enQtNDAwNzE4Mjk2NDE3LTJhOTlmZjM5YzkwNmEzNmY3ZjE2MTU2OTMxOGE4ZDM0MmU4ZWRkZGJiODgzNmRmMTYxMDYwNjZiMDAwMTA2OWM)
