...
export default async (coreConfig: CoreConfig) => {
  ...
  if(err) throw new VError(err, `while cloning ${coreConfig.organization}/${coreConfig.repository}`);

  [err] = await to(CopyDocs.releases({
    releases,
    source: tempPath,
    output: outputPath,
  }));
  ...
  [err] = await to(CopyDocs.branches({
    branches,
    source: tempPath,
    output: outputPath,
  }));
  ...
  [err] = await to(DocsVersions.generate({
    releases,
