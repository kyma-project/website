...
export class CapabilitiesFetcher {
  ...
  copyCapabilities = async (capabilitiesDir: string, output: string) => {
    ...
    const [err] = await to(copyResources(capabilitiesDir, output, this.allowedFilesRegex));
    if (err) throw new VError(err, `while copying capabilities to ${output}`);
  }

  extractCapabilitiesMetadata = async (capabilitiesDir: string): Promise<Attributes[]> => {
    let err: Error | null;
    let files: any;
    ...
    let filteredFiles = files
      .filter(file => Boolean(this.allowedFilesRegex.exec(file)));

    const extractedMetadata = filteredFiles.map(async (file: string): Promise<Attributes> => {
    ...
    if (err) throw new VError(err, `while extracting metadata from capabilities`);
